...
public class InnerDeclarationRemovedReaction extends AbstractReactionRealization {
  private RemoveEReference<CompositeDataType, InnerDeclaration> removeChange;
  
  private int currentlyMatchedChange;
  
  public InnerDeclarationRemovedReaction(final RoutinesFacade routinesFacade) {
  ...
  public void executeReaction(final EChange change) {
    ...
    mir.reactions.pcmInnerDeclarationReactions.InnerDeclarationRemovedReaction.ActionUserExecution userExecution = new mir.reactions.pcmInnerDeclarationReactions.InnerDeclarationRemovedReaction.ActionUserExecution(this.executionState, this);
  ...
  private boolean checkUserDefinedPrecondition(final RemoveEReference removeChange, final CompositeDataType affectedEObject, final EReference affectedFeature, final InnerDeclaration oldValue, final int index) {
    boolean _contains = affectedEObject.getInnerDeclaration_CompositeDataType().contains(oldValue);
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public void callRoutine1(final RemoveEReference removeChange, final CompositeDataType affectedEObject, final EReference affectedFeature, final InnerDeclaration oldValue, final int index, @Extension final RoutinesFacade _routinesFacade) {
      _routinesFacade.removeCorrespondingAttribute(oldValue, affectedEObject);
    }
  }
}
