...
public class SignatureRemovedFromInterfaceReaction extends AbstractReactionRealization {
  private RemoveEReference<OperationInterface, OperationSignature> removeChange;
  
  private int currentlyMatchedChange;
  
  public SignatureRemovedFromInterfaceReaction(final RoutinesFacade routinesFacade) {
  ...
  public void executeReaction(final EChange change) {
    ...
    mir.reactions.pcmSignatureReactions.SignatureRemovedFromInterfaceReaction.ActionUserExecution userExecution = new mir.reactions.pcmSignatureReactions.SignatureRemovedFromInterfaceReaction.ActionUserExecution(this.executionState, this);
  ...
  private boolean checkUserDefinedPrecondition(final RemoveEReference removeChange, final OperationInterface affectedEObject, final EReference affectedFeature, final OperationSignature oldValue, final int index) {
    boolean _contains = affectedEObject.getSignatures__OperationInterface().contains(oldValue);
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public void callRoutine1(final RemoveEReference removeChange, final OperationInterface affectedEObject, final EReference affectedFeature, final OperationSignature oldValue, final int index, @Extension final RoutinesFacade _routinesFacade) {
      _routinesFacade.removeCorrespondingOperation(oldValue, affectedEObject);
    }
  }
}
