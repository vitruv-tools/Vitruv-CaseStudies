...
public class RequiredRoleRemovedReaction extends AbstractReactionRealization {
  private RemoveEReference<InterfaceProvidingRequiringEntity, OperationRequiredRole> removeChange;
  
  private int currentlyMatchedChange;
  
  public RequiredRoleRemovedReaction(final RoutinesFacade routinesFacade) {
  ...
  public void executeReaction(final EChange change) {
    ...
    mir.reactions.pcmRequiredRoleReactions.RequiredRoleRemovedReaction.ActionUserExecution userExecution = new mir.reactions.pcmRequiredRoleReactions.RequiredRoleRemovedReaction.ActionUserExecution(this.executionState, this);
  ...
  private boolean matchRemoveChange(final EChange change) {
    if (change instanceof RemoveEReference<?, ?>) {
    	...
    	this.removeChange = (RemoveEReference<org.palladiosimulator.pcm.core.entity.InterfaceProvidingRequiringEntity, org.palladiosimulator.pcm.repository.OperationRequiredRole>) change;
    	return true;
    }
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public void callRoutine1(final RemoveEReference removeChange, final InterfaceProvidingRequiringEntity affectedEObject, final EReference affectedFeature, final OperationRequiredRole oldValue, final int index, @Extension final RoutinesFacade _routinesFacade) {
      _routinesFacade.removeCorrespondingRequiredElements(oldValue, affectedEObject);
    }
  }
}
