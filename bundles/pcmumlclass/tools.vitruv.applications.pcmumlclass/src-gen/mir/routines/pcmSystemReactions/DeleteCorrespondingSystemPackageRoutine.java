...
public class DeleteCorrespondingSystemPackageRoutine extends AbstractRepairRoutineRealization {
  private DeleteCorrespondingSystemPackageRoutine.ActionUserExecution userExecution;
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public void callRoutine1(final org.palladiosimulator.pcm.system.System pcmSystem, final Optional<org.eclipse.uml2.uml.Package> umlSystemPackage, final Optional<org.eclipse.uml2.uml.Class> umlSystemImplementation, final Optional<Operation> umlSystemConstructor, @Extension final RoutinesFacade _routinesFacade) {
      ...
      if (_isPresent) {
        _routinesFacade.deleteCorrespondencToSystemPackage(pcmSystem);
      }
      boolean _isPresent_1 = umlSystemImplementation.isPresent();
      if (_isPresent_1) {
        _routinesFacade.deleteCorrespondencToSystemImplementation(pcmSystem);
      }
      boolean _isPresent_2 = umlSystemConstructor.isPresent();
      if (_isPresent_2) {
        _routinesFacade.deleteCorrespondencToSystemConstructor(pcmSystem);
      }
    }
  }
  
  public DeleteCorrespondingSystemPackageRoutine(final RoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final org.palladiosimulator.pcm.system.System pcmSystem) {
    super(routinesFacade, reactionExecutionState, calledBy);
    this.userExecution = new mir.routines.pcmSystemReactions.DeleteCorrespondingSystemPackageRoutine.ActionUserExecution(getExecutionState(), this);
  ...
  protected boolean executeRoutine() throws IOException {
    getLogger().debug("Called routine DeleteCorrespondingSystemPackageRoutine with input:");
