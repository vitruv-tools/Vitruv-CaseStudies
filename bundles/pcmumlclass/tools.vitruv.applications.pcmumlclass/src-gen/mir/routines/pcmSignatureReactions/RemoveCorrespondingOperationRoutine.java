...
public class RemoveCorrespondingOperationRoutine extends AbstractRepairRoutineRealization {
  private RemoveCorrespondingOperationRoutine.ActionUserExecution userExecution;
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public void update0Element(final OperationSignature pcmSignature, final OperationInterface pcmInterface, final Interface umlInterface, final Operation umlOperation) {
      ...
      _ownedOperations.remove(umlOperation);
    }
  ...
  public RemoveCorrespondingOperationRoutine(final RoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final OperationSignature pcmSignature, final OperationInterface pcmInterface) {
    super(routinesFacade, reactionExecutionState, calledBy);
    this.userExecution = new mir.routines.pcmSignatureReactions.RemoveCorrespondingOperationRoutine.ActionUserExecution(getExecutionState(), this);
  ...
  protected boolean executeRoutine() throws IOException {
    getLogger().debug("Called routine RemoveCorrespondingOperationRoutine with input:");
