package mir.reactions.pcm2depInjectJava.pcm2javaCommon;
...
public class AddedAssemblyContextToComposedStructureReaction extends AbstractReactionRealization {
  ...
  public void executeReaction(final EChange change) {
    ...
    mir.reactions.pcm2depInjectJava.pcm2javaCommon.AddedAssemblyContextToComposedStructureReaction.ActionUserExecution userExecution = new mir.reactions.pcm2depInjectJava.pcm2javaCommon.AddedAssemblyContextToComposedStructureReaction.ActionUserExecution(this.executionState, this);
  ...
  private boolean checkUserDefinedPrecondition(final InsertEReference insertChange, final ComposedStructure affectedEObject, final EReference affectedFeature, final AssemblyContext newValue, final int index) {
    ...
    for (final AssemblyContext ac : assemblyContexts) {
      if (((!Objects.equal(ac, newValue)) && ac.getEncapsulatedComponent__AssemblyContext().equals(component))) {
        ...
        this.userInteracting.getNotificationDialogBuilder().message(msg).windowModality(WindowModality.MODAL).showDialogAndGetUserInput();
        return false;
      }
    }
    return true;
