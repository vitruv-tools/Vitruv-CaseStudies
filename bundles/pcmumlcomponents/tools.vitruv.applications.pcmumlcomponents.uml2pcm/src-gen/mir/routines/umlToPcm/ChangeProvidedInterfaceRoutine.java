...
public class ChangeProvidedInterfaceRoutine extends AbstractRepairRoutineRealization {
    ...
    public void update0Element(final InterfaceRealization interfaceRealization, final Interface umlInterface, final OperationProvidedRole pcmRole, final OperationInterface pcmInterface) {
      ...
      if (_equals) {
        ...
        if (_equals_1) {
          ...
          this.userInteractor.getNotificationDialogBuilder().message("Further interfaces will not be provided in the PCM").windowModality(UserInteractionOptions.WindowModality.MODAL).startInteraction();
        }
      }
    }
  ...
  public ChangeProvidedInterfaceRoutine(final RoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final InterfaceRealization interfaceRealization, final Interface umlInterface) {
