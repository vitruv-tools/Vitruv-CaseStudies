...
public class ChangeRequiredInterfaceRoutine extends AbstractRepairRoutineRealization {
    ...
    public void update0Element(final Usage umlUsage, final Interface umlInterface, final OperationInterface pcmInterface, final OperationRequiredRole pcmRole) {
      ...
      if (_equals) {
        ...
        if (_equals_1) {
          ...
          this.userInteractor.getNotificationDialogBuilder().message("Further interfaces will not be required in the PCM").windowModality(UserInteractionOptions.WindowModality.MODAL).startInteraction();
        }
      }
    }
  ...
  public ChangeRequiredInterfaceRoutine(final RoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final Usage umlUsage, final Interface umlInterface) {
