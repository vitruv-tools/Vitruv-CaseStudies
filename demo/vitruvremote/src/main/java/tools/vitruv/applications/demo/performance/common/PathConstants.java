package tools.vitruv.applications.demo.performance.common;

public final class PathConstants {
    public static final String PATH_TRUST_STORE = "/truststore";

    public static final String PATH_SERVER_START = "/server/start";

    public static final String PATH_SERVER_STOP = "/server/stop";

    public static final String PATH_CLIENT_START = "/client/start";

    public static final String PATH_CLIENT_STATE = "/client/state";

    public static final String PATH_PERFORMANCE_DATA = "/data";

    public static final String PATH_WORKER_STOP = "/worker/stop";

    public static final String PATH_GET_OIDC = "/oidc";

    public static final String QUERY_PARAMETER_SERVER_CONFIG = "serverconfig";

    private PathConstants() {}
}
