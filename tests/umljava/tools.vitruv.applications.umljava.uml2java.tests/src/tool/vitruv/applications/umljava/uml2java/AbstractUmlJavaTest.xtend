...
class AbstractUmlJavaTest extends VitruviusChangePropagationTest {
     ...
     * @param uType Uml-Typ
     */
    def protected assertJavaMemberHasType(org.emftext.language.java.types.TypedElement jMem, org.eclipse.uml2.uml.Classifier uType) {
        if (uType instanceof org.eclipse.uml2.uml.PrimitiveType) {
            assertEquals(uType.name, jMem.typeReference.class.simpleName)
    ...
    protected def <T extends Modifier> assertJavaModifiableHasModifier(AnnotableAndModifiable jMem, Class<T> mod) {
     ...
     */
    def protected org.emftext.language.java.classifiers.Classifier getClassifierfromTypeRef(TypeReference type) {
        return (type as NamespaceClassifierReference).classifierReferences.head.target;
    }
