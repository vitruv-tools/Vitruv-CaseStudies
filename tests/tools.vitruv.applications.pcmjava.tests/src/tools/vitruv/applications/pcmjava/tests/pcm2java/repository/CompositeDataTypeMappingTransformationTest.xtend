...
class CompositeDataTypeMappingTransformationTest extends Pcm2JavaTransformationTest {
	...
	def void testAddPrimitiveTypeToCompositeDataType() {
		...
		changePcmView[
			claimSinglePcmRepository(it) => [
				...
				dataType.innerDeclaration_CompositeDataType +=
					createInnerDeclaration(innerType, Pcm2JavaTestUtils.INNER_DEC_NAME)
			]
		]
	...
	def void testAddCompositeDataTypeToCompositeDataType() {
		...
		changePcmView[
			claimSinglePcmRepository(it) => [
				val outerCompositeDataType = dataTypes__Repository.filter(CompositeDataType).filter(
					ctd |
						ctd.entityName == Pcm2JavaTestUtils.COMPOSITE_DATA_TYPE_NAME
				).claimOne
				val innerCompositeDataType = dataTypes__Repository.filter(CompositeDataType).filter(
					ctd |
						ctd.entityName == INNER_COMPOSITE_DATATYPE_NAME
				).claimOne
				outerCompositeDataType.innerDeclaration_CompositeDataType +=
					createInnerDeclaration(innerCompositeDataType, Pcm2JavaTestUtils.INNER_DEC_NAME)
			]
		]
		...
		validateJavaView[
			...
			assertExistenceOfCompilationUnitsDeeplyEqualTo(
				List.of(innerDataTypeExpectedCompilationUnit, outerDataTypeExpectedCompilationUnit))
		]
	}
