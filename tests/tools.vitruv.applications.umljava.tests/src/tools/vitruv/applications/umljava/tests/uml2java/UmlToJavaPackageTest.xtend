...
class UmlToJavaPackageTest extends UmlJavaTransformationTest {
	...
	def void testCreatePackage() {
		...
		createUmlAndJavaPackagesView() => [
			val umlPackage = uniqueUmlModel.getUniqueUmlPackageWithName(PACKAGE_NAME)
	...
	def void testCreateUppercasePackage() {
		...
		createUmlAndJavaPackagesView() => [
			val umlPackage = uniqueUmlModel.getUniqueUmlPackageWithName(PACKAGE_NAME.toFirstUpper)
			val javaPackage = getUniqueJavaPackageWithName(PACKAGE_NAME)
			assertEquals(1, javaPackages.size, "exactly one Java package should exist")
			assertPackageEquals(umlPackage, javaPackage)
		]
	...
	def void testCreateNestedPackage() {
		...
		createUmlAndJavaPackagesView() => [
			val umlRootPackage = uniqueUmlModel.getUniqueUmlPackageWithName(PACKAGE_NAME)
			val umlNestedPackage = umlRootPackage.getUniqueUmlPackageWithName(NESTED_PACKAGE_NAME)
			val javaRootPackage = getUniqueJavaPackageWithName(PACKAGE_NAME)
			val javaNestedPackage = getUniqueJavaPackageWithName(NESTED_PACKAGE_NAME)
			...
			assertPackageEquals(umlRootPackage, javaRootPackage)
			assertPackageEquals(umlNestedPackage, javaNestedPackage)
		]
	}
	...
	def void testCreateNestedUppercasePackage() {
		...
		createUmlAndJavaPackagesView() => [
			val umlRootPackage = uniqueUmlModel.getUniqueUmlPackageWithName(PACKAGE_NAME.toFirstUpper)
			val umlNestedPackage = umlRootPackage.getUniqueUmlPackageWithName(NESTED_PACKAGE_NAME.toFirstUpper)
			val javaRootPackage = getUniqueJavaPackageWithName(PACKAGE_NAME)
			val javaNestedPackage = getUniqueJavaPackageWithName(NESTED_PACKAGE_NAME)
			assertEquals(PACKAGE_NAME, javaNestedPackage.namespaces.join("."))
			assertEquals(2, javaPackages.size, "exactly two Java packages should exist")
	...
	def void testRenamePackage() {
		createRootPackage(PACKAGE_NAME)
		changeUmlModel [
			getUniqueUmlPackageWithName(PACKAGE_NAME) => [
				name = PACKAGE_RENAMED
			]
		]
		createUmlAndJavaPackagesView() => [
			val umlPackage = uniqueUmlModel.getUniqueUmlPackageWithName(PACKAGE_RENAMED)
			val javaPackage = getUniqueJavaPackageWithName(PACKAGE_RENAMED)
			assertEquals(1, javaPackages.size, "exactly one Java package should exist")
			assertPackageEquals(umlPackage, javaPackage)
		]
	}
	...
	def void testMovePackage() {
		...
		changeUmlModel[
			val toBeNestedPackage = getUniqueUmlPackageWithName(NESTED_PACKAGE_NAME)
			getUniqueUmlPackageWithName(PACKAGE_NAME) => [
	...
	def void testDeletePackage() {
		createRootPackage(PACKAGE_NAME)
		changeUmlModel[
			getUniqueUmlPackageWithName(PACKAGE_NAME).destroy
		]
		createUmlAndJavaPackagesView() => [
