...
class MethodMappingTransformationTest extends Java2PcmTransformationTest {
	...
	void testAddMethod() throws Exception {
		...
		validatePcmView(view -> {
			...
			OperationInterface operationInterface = claimInterface(repository, Pcm2JavaTestUtils.INTERFACE_NAME, OperationInterface.class);
			OperationSignature operationSignature = claimNamedElement(operationInterface.getSignatures__OperationInterface(), Pcm2JavaTestUtils.OPERATION_SIGNATURE_1_NAME);
	...
	void testRenameMethod() throws Exception {
		...
		changeJavaEditorView(view -> {
			...
			String newMethodName = Pcm2JavaTestUtils.OPERATION_SIGNATURE_1_NAME + Pcm2JavaTestUtils.RENAME;
		...
		validatePcmView(view -> {
			Repository repository = claimSingleRepository(view);
			OperationInterface operationInterface = claimInterface(repository, Pcm2JavaTestUtils.INTERFACE_NAME, OperationInterface.class);
			claimNamedElement(operationInterface.getSignatures__OperationInterface(), Pcm2JavaTestUtils.OPERATION_SIGNATURE_1_NAME + Pcm2JavaTestUtils.RENAME);
			assertEquals(1, operationInterface.getSignatures__OperationInterface().size(), "too many operation signatures");
		});
	}
	...
	void testAddReturnType() throws Exception {
		...
		validatePcmView(view -> {
			...
			OperationSignature operationSignature = claimNamedElement(operationInterface.getSignatures__OperationInterface(), Pcm2JavaTestUtils.OPERATION_SIGNATURE_1_NAME);
			assertEquals(0, operationSignature.getParameters__OperationSignature().size());
			assertEquals(changedReturnType, getTypeNameOfPcmDataType(operationSignature.getReturnType__OperationSignature()));
		});
	}
}
